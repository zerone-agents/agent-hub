...
func (h *AdminUserHandler) ListUsers(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusInternalServerError, ErrCodeUserQueryFailed, "查询用户失败")
		return
	}
...
func (h *AdminUserHandler) UpdateUser(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusBadRequest, ErrCodeInvalidUserId, "无效的用户 ID")
		return
	}
	...
	if req.Role == "" && req.Status == "" {
		respondError(c, http.StatusBadRequest, ErrCodeFieldRequired, "至少需要提供一个字段")
		return
	}
	if req.Role != "" {
		rcpt, err := h.users.UpdateRole(id, actorID, req.Role)
		if err != nil {
			respondError(c, http.StatusBadRequest, ErrCodeUserRoleUpdateFailed, err.Error())
			return
		}
	...
	if req.Status != "" {
		...
		if err != nil {
			respondError(c, http.StatusBadRequest, ErrCodeUserStatusUpdateFailed, err.Error())
			return
		}
...
func (h *AdminUserHandler) ResetUserPassword(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusBadRequest, ErrCodePasswordResetFailed, err.Error())
		return
	}
	_ = h.provider.RevokeAllForUser(id)
...
func (h *AdminUserHandler) CreateInvite(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusBadRequest, ErrCodeInviteCreationFailed, err.Error())
		return
	}
...
func (h *AdminUserHandler) ListInvites(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusInternalServerError, ErrCodeInviteQueryFailed, "查询邀请失败")
		return
	}
...
func (h *AdminUserHandler) RevokeInvite(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil {
		respondError(c, http.StatusBadRequest, ErrCodeInvalidInviteId, "无效的邀请 ID")
		return
	}
	if err := h.invites.Revoke(id); err != nil {
		respondError(c, http.StatusBadRequest, ErrCodeInviteRevokeFailed, err.Error())
		return
	}
