...
func (h *AgentDetailHandler) GetAgentDetail(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusConflict, ErrCodeAgentUnavailable, "agent not available: "+err.Error())
		return
	}
	...
	if err != nil {
		...
		if errors.As(err, &httpErr) && httpErr.StatusCode == http.StatusNotFound {
			respondError(c, http.StatusNotFound, ErrCodeAgentRuntimeNotFound, "Agent 在运行时不存在")
			return
		}
		log.Printf("agent detail for %q failed: %v", agentName, err)
		respondError(c, http.StatusBadGateway, ErrCodeAgentRuntimeUnavailable, "Agent 运行时不可用")
		return
	}
	...
	if err != nil {
		...
		respondError(c, http.StatusBadGateway, ErrCodeAgentDetailUnparseable, "Agent 运行时返回了无法解析的详情数据")
		return
	}
