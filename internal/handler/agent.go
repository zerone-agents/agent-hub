...
func respondAgentError(c *gin.Context, err error) {
	...
	case errors.Is(err, provider.ErrProviderNotFound):
		...
		respondError(c, http.StatusNotFound, ErrCodeProviderNotFound, "Provider 不存在")
	default:
...
func (h *AgentHandler) Delete(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusBadGateway, ErrCodeDeploymentStatusUncertain, "无法确认部署状态，已阻止删除")
		return
	}
...
func (h *AgentHandler) DeployAgent(c *gin.Context) {
	...
	if err != nil {
		respondError(c, deployerErrorStatus(err), ErrCodeDeployFailed, deployerErrorMessage(err))
		return
	}
...
func (h *AgentHandler) StopDeployment(c *gin.Context) {
	...
	if err := h.deployerService.Stop(tenant.GetTenantID(c), name); err != nil {
		respondError(c, deployerErrorStatus(err), ErrCodeDeployStopFailed, deployerErrorMessage(err))
		return
	}
...
func (h *AgentHandler) StartDeployment(c *gin.Context) {
	...
	if err != nil {
		respondError(c, deployerErrorStatus(err), ErrCodeDeployStartFailed, deployerErrorMessage(err))
		return
	}
...
func (h *AgentHandler) DeleteDeployment(c *gin.Context) {
	...
	if err != nil {
		respondError(c, deployerErrorStatus(err), ErrCodeDeployDeleteFailed, deployerErrorMessage(err))
		return
	}
