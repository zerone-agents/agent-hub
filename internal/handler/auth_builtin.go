...
func (h *BuiltinAuthHandler) GetMode(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusInternalServerError, ErrCodeSetupStatusFailed, "查询初始化状态失败")
		return
	}
...
func (h *BuiltinAuthHandler) Setup(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		respondError(c, http.StatusBadRequest, ErrCodeIncompleteParameter, "参数不完整")
		return
	}
	if req.Password != req.ConfirmPassword {
		respondError(c, http.StatusBadRequest, ErrCodePasswordMismatch, "两次输入的密码不一致")
		return
	}
	user, err := h.users.CreateInitialAdmin(req.Password)
	if errors.Is(err, services.ErrAlreadyInitialized) {
		respondError(c, http.StatusConflict, ErrCodeAlreadyInitialized, "系统已初始化")
		return
	}
	if err != nil {
	...
	if err != nil {
		respondError(c, http.StatusInternalServerError, ErrCodeIssueTokenFailed, "签发令牌失败")
		return
	}
...
func (h *BuiltinAuthHandler) Login(c *gin.Context) {
	...
	if err != nil {
		...
		if errors.Is(err, services.ErrLocked) {
			respondError(c, http.StatusTooManyRequests, ErrCodeRateLimited, err.Error())
			return
		}
		respondError(c, http.StatusUnauthorized, ErrCodeInvalidCredentials, services.ErrInvalidCredentials.Error())
		return
	}
	pair, err := h.p.IssueTokenPair(user)
	if err != nil {
...
func (h *BuiltinAuthHandler) Refresh(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		respondError(c, http.StatusBadRequest, ErrCodeRefreshTokenRequired, "refresh token is required")
		return
	}
	...
	if err != nil {
		respondError(c, http.StatusUnauthorized, ErrCodeInvalidRefreshToken, "refresh token 无效或已过期")
		return
	}
	respondSuccess(c, pair)
}
...
func (h *BuiltinAuthHandler) Register(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusGone, ErrCodeInvalidInvite, services.ErrInviteInvalid.Error())
		return
	}
...
func (h *BuiltinAuthHandler) ChangePassword(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusUnauthorized, ErrCodeInvalidUserIdentity, "无效的用户身份")
		return
	}
	if err := h.users.ChangePassword(id, req.OldPassword, req.NewPassword); err != nil {
		...
		respondError(c, status, ErrCodeLoginFailed, err.Error())
		return
	}
	...
	if err != nil {
		respondError(c, http.StatusInternalServerError, ErrCodeUserQueryFailed, "用户查询失败")
		return
	}
	pair, err := h.p.IssueTokenPair(user)
	if err != nil {
