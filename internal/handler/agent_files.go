...
func (h *AgentFilesHandler) proxy(c *gin.Context, method, runtimePath string) {
	...
	if err != nil {
		respondError(c, http.StatusBadGateway, ErrCodeRuntimeUnreachable, "runtime unreachable: "+err.Error())
		return
	}
	...
	if resp.StatusCode >= 500 {
		...
		resp.Body.Close()
		respondError(c, http.StatusBadGateway, ErrCodeRuntimeUnreachable, "runtime unreachable: HTTP "+strconv.Itoa(resp.StatusCode)+": "+string(body))
		return
	}
