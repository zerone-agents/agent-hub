...
func (h *CasdoorUserHandler) ListUsers(c *gin.Context) {
	...
	if err != nil {
		respondError(c, http.StatusBadGateway, ErrCodeCasdoorUserQueryFailed, "Casdoor 用户查询失败: "+err.Error())
		return
	}
...
func (h *CasdoorUserHandler) UpdateUser(c *gin.Context) {
	...
	if req.Status != "" && req.Status != "active" && req.Status != "disabled" {
		respondError(c, http.StatusBadRequest, ErrCodeInvalidStatus, "无效的 status")
		return
	}
	if req.Role != "" {
...
func (h *CasdoorUserHandler) LoginURL(c *gin.Context) {
	...
	if err != nil {
		...
		respondError(c, http.StatusBadGateway, ErrCodeLoginLinkFailed, "生成登录链接失败: "+err.Error())
		return
	}
...
func respondDirectoryError(c *gin.Context, err error) {
	...
	case errors.Is(err, directory.ErrSelfOperation):
		respondError(c, http.StatusBadRequest, ErrCodeSelfOperationForbidden, err.Error())
	case errors.Is(err, directory.ErrInvalidRole):
		respondError(c, http.StatusBadRequest, ErrCodeInvalidRole, err.Error())
	case errors.Is(err, directory.ErrUserNotFound):
		respondError(c, http.StatusNotFound, ErrCodeUserNotFound, err.Error())
	default:
		respondError(c, http.StatusBadGateway, ErrCodeCasdoorOperationFailed, "Casdoor 操作失败: "+err.Error())
	}
}
