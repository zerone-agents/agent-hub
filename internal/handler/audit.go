...
func (h *AuditHandler) List(c *gin.Context) {
	...
	if err != nil || page < 1 {
		respondError(c, http.StatusBadRequest, "invalid_pagination", "无效的分页参数")
		return
	}
	...
	if s := c.Query("from"); s != "" {
		...
		if err != nil {
			respondError(c, http.StatusBadRequest, "invalid_time_range", "无效的时间范围")
			return
		}
		f.From = &tv
	}
	...
	if s := c.Query("snapshotId"); s != "" {
		...
		if err != nil {
			respondError(c, http.StatusBadRequest, "invalid_snapshot_parameter", "无效的快照参数")
			return
		}
		snapshot = &v
	}
	...
	if err != nil {
		respondError(c, http.StatusInternalServerError, "audit_query_failed", "查询审计日志失败")
		return
	}
