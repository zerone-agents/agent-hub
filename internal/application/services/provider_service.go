...
func (s *ProviderService) ListAll(typeFilter string) ([]provider.Provider, error) {
	...
	allModels, err := s.repo.ListAllModels()
	if err != nil {
	...
	for _, summary := range summaries {
		...
		repaired, err := s.repairSelectionIDs(p)
		if err != nil {
			return nil, err
		}
...
// storage layer — but kept as defense-in-depth.
func (s *ProviderService) repairSelectionIDs(p provider.Provider) (provider.Provider, error) {
	...
	if err := s.repo.ReplaceModels(p.ID(), rows); err != nil {
...
func (s *ProviderService) GetByID(id uint64) (provider.Provider, error) {
	...
	return s.repairSelectionIDs(p)
}

// GetProviderByID is an alias for GetByID; it returns provider.ErrProviderNotFound
// when the DB row is missing.
func (s *ProviderService) GetProviderByID(id uint64) (provider.Provider, error) {
	return s.GetByID(id)
}

// ── Create ──────────────────────────────────────────────────────

func (s *ProviderService) Create(input *CreateProviderInput) (*ProviderDTO, error) {
	...
	exists, err := s.repo.ExistsByKey(input.Key)
	if err != nil {
	...
	if len(input.DefaultModels) > 0 {
		...
		rows, err = s.assignBulkAigcCodes(summary.ID, rows)
		if err != nil {
			return nil, err
		}
		if err := s.repo.ReplaceModels(summary.ID, rows); err != nil {
	...
	rows, err := s.repo.ListModels(summary.ID)
	if err != nil {
		return nil, fmt.Errorf("加载 provider_models 失败: %w", err)
	}
	...
	if len(input.Attributes) > 0 {
		if err := s.repo.SetAttributes(p.ID(), input.Attributes); err != nil {
...
func (s *ProviderService) Update(id uint64, input *UpdateProviderInput) (*ProviderDTO, error) {
	summary, err := s.repo.GetByID(id)
	if err != nil {
		return nil, provider.ErrProviderNotFound
	}
	...
	if input.DefaultModels != nil {
		...
		rows, err = s.assignBulkAigcCodes(id, rows)
		if err != nil {
			return nil, err
		}
		if err := s.repo.ReplaceModels(id, rows); err != nil {
	...
	if err := s.repo.Update(summary); err != nil {
	...
	rows, err := s.repo.ListModels(p.ID())
	if err != nil {
		return nil, fmt.Errorf("加载 provider_models 失败: %w", err)
	}
...
func (s *ProviderService) ToDTO(p provider.Provider) (*ProviderDTO, error) {
	models := p.DefaultModels()
	if models == nil {
		models = []provider.CatalogModel{}
	}
	...
	attributes, err := s.repo.GetAttributes(p.ID())
	if err != nil {
...
func (s *ProviderService) GetRawAPIKey(id uint64) (string, error) {
	summary, err := s.repo.GetByID(id)
	if err != nil {
		return "", provider.ErrProviderNotFound
	}
...
func (s *ProviderService) RevealAPIKey(id uint64) (string, error) {
	summary, err := s.repo.GetByID(id)
	if err != nil {
		return "", provider.ErrProviderNotFound
	}
...
func (s *ProviderService) ListRuntimeConfigs() ([]*ProviderRuntimeConfig, error) {
	summaries, err := s.repo.ListAll()
	if err != nil {
		return nil, fmt.Errorf("获取 Provider 列表失败: %w", err)
	}
...
func (s *ProviderService) SyncProviderToMultiRAG(ctx context.Context, providerID uint64, client provider.MultiRAGClient, verifyOnly bool, modelIds []string) (*provider.SyncResult, error) {
	if client == nil {
		return nil, provider.ErrMultiRAGConfigMissing
	}
	p, err := s.GetByID(providerID)
	if err != nil {
		return nil, provider.ErrProviderNotFound
	}
...
// its DTO in one call.
func (s *ProviderService) GetByIDAsDTO(providerID uint64) (*ProviderDTO, error) {
	p, err := s.GetByID(providerID)
	if err != nil {
		return nil, err
	}
...
func (s *ProviderService) FindProviderByModelID(modelID string) (provider.Provider, error) {
	if modelID == "" {
		return nil, provider.ErrProviderNotFound
	}
	providers, err := s.ListAll("")
	if err != nil {
		return nil, err
	}
...
func (s *ProviderService) AddModel(providerID uint64, input *AddModelInput) (*ProviderDTO, error) {
	if _, err := s.repo.GetByID(providerID); err != nil {
		return nil, provider.ErrProviderNotFound
	}
	if input.ModelID == "" {
	...
	existingRows, err := s.repo.ListModels(providerID)
	if err != nil {
		return nil, fmt.Errorf("加载 provider_models 失败: %w", err)
	}
	maxSortOrder := 0
	...
	allRows, err := s.repo.ListAllModels()
	if err != nil {
	...
	if err := s.repo.CreateModel(row); err != nil {
		return nil, fmt.Errorf("创建 model 失败: %w", err)
	}
	return s.GetByIDAsDTO(providerID)
}

// UpdateModel applies a partial update to a single model row identified by
// (providerID, selectionID) and returns the updated provider DTO.
func (s *ProviderService) UpdateModel(providerID uint64, selectionID string, input *UpdateModelInput) (*ProviderDTO, error) {
	row, err := s.repo.GetModelBySelectionID(providerID, selectionID)
	if err != nil {
		return nil, provider.ErrProviderNotFound
	}
	...
	if err := s.repo.UpdateModel(row); err != nil {
...
func (s *ProviderService) DeleteModel(providerID uint64, selectionID string) error {
	if _, err := s.repo.GetModelBySelectionID(providerID, selectionID); err != nil {
		return provider.ErrProviderNotFound
	}
	return s.repo.DeleteModel(providerID, selectionID)
}
...
func (s *ProviderService) assignBulkAigcCodes(providerID uint64, rows []provider.ProviderModel) ([]provider.ProviderModel, error) {
	ownRows, err := s.repo.ListModels(providerID)
	if err != nil {
