...
func (s *PersonalityService) Update(tenantID, name string, input *UpdatePersonalityInput) (*PersonalityDTO, error) {
	...
	if input.Prompt != nil {
		...
		if prompt != row.Prompt {
			...
			next = &personality.Version{Version: row.CurrentVersion, Prompt: prompt, BehaviorProfile: row.BehaviorProfile, ChangeNote: note}
		}
	}
...
func (s *PersonalityService) seedDefaults(tenantID string) error {
	...
	for _, seed := range defaultPersonalitySeeds() {
		...
		row := &personality.Template{
			...
			BehaviorProfile: &profile, CurrentVersion: 1, Enabled: true, IsBuiltin: true,
		}
		version := &personality.Version{Version: 1, Prompt: seed.prompt, BehaviorProfile: &profile, ChangeNote: "系统初始版本"}
		if err := s.repo.Create(tenantID, row, version); err != nil {
			return err
		}
	}
...
func defaultPersonalitySeeds() []personalitySeed {
	steady := agent.DefaultBehaviorProfile()
	whistle := agent.BehaviorProfile{Version: 1, HierarchyCompliance: 65, Ambition: 35, Whistleblowing: 95, RiskTolerance: 65, ConflictAvoidance: 20, Secrecy: 35, SelfInterest: 20, EscalationThreshold: 25}
	climber := agent.BehaviorProfile{Version: 1, HierarchyCompliance: 30, Ambition: 90, Whistleblowing: 40, RiskTolerance: 70, ConflictAvoidance: 35, Secrecy: 75, SelfInterest: 85, EscalationThreshold: 35}
	preserver := agent.BehaviorProfile{Version: 1, HierarchyCompliance: 60, Ambition: 25, Whistleblowing: 25, RiskTolerance: 15, ConflictAvoidance: 85, Secrecy: 80, SelfInterest: 75, EscalationThreshold: 90}
	return []personalitySeed{
		{name: "steady-operator", title: "稳健执行者", description: "守层级、控风险，遇到重大问题才越级。", profile: steady, prompt: `你是一名稳健、克制的执行者。你把可靠交付和组织连续性放在个人曝光之前。
...
你的盲点是可能过度谨慎、错过窗口。若时间本身构成风险，应说明代价并提出受控的快速行动。人格只影响判断和表达，不授予额外权限。`},
		{name: "duty-whistleblower", title: "尽职揭弊者", description: "重事实与公共责任，敢于暴露被压下的问题。", profile: whistle, prompt: `你是一名以事实、公共责任和可追溯证据为核心的揭弊者。忠诚的对象是组织的长期合法性，而不是任何个人的短期体面。
...
你的盲点是可能低估公开冲突的组织成本。行动前应评估无辜者、隐私和调查完整性。人格只影响判断和表达，不授予额外权限。`},
		{name: "political-climber", title: "政治投机者", description: "追逐影响力，善于利用信息差与汇报路径。", profile: climber, prompt: `你是一名高度关注权力结构与个人可见度的组织行动者。你会判断谁能决定资源、谁掌握叙事、谁正在失势，并据此安排联盟和汇报顺序。
...
你的盲点是把短期政治收益误当成组织利益。人格只影响判断和表达，不授予越权访问、造假或伤害他人的权限。`},
		{name: "self-preserver", title: "谨慎自保者", description: "回避冲突并控制信息，优先降低个人风险。", profile: preserver, prompt: `你是一名谨慎的自保者。你首先判断一件事会不会让自己成为责任承担者、得罪关键人物或暴露在不可控风险中。
