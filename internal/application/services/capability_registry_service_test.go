...
func TestCapabilityRegistryControlsRunBindingAndLocksSnapshot(t *testing.T) {
	...
	require.NoError(t, db.AutoMigrate(&capability.Package{}, &capability.ResourceProvenance{}, &rundomain.Run{}, &rundomain.RunAgent{}, &rundomain.CapabilityBinding{}))
	...
	require.Equal(t, "pending", p.ApprovalStatus)
	_, err = registry.SetEnabled("tenant-a", p.ID, true)
	require.ErrorIs(t, err, ErrCapabilityApprovalRequired)
	_, err = registry.Approve("tenant-a", p.ID, "admin-1", nil)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = registry.Approve("tenant-a", v2.ID, "admin-1", nil)
	require.NoError(t, err)
...
func TestCapabilityRegistryRejectsInvalidAndDuplicateManifest(t *testing.T) {
	db, err := gorm.Open(sqlite.Open("file:"+t.Name()+"?mode=memory&cache=shared"), &gorm.Config{})
	require.NoError(t, err)
	require.NoError(t, db.AutoMigrate(&capability.Package{}, &capability.ResourceProvenance{}))
...
func TestCapabilityRegistryRecordsPermissionApprovalAndResourceProvenance(t *testing.T) {
	db, err := gorm.Open(sqlite.Open("file:"+t.Name()+"?mode=memory&cache=shared"), &gorm.Config{})
	require.NoError(t, err)
	require.NoError(t, db.AutoMigrate(&capability.Package{}, &capability.ResourceProvenance{}))
	s := NewCapabilityRegistryService(db)
	manifest := strings.Replace(testCapabilityManifest, "stateSchemas: []", "stateSchemas:\n    - id: task\n      version: 1.0.0\n      file: schemas/task.json", 1)
	p, err := s.Register("t", RegisterCapabilityInput{ManifestYAML: manifest, SourceType: "registry", SourceRef: "https://packages.example/task-state"})
	require.NoError(t, err)
	require.Equal(t, "registry", p.SourceType)
	require.NotEmpty(t, p.RequestedPermissions)
	resources, err := s.Resources("t", p.ID)
	require.NoError(t, err)
	require.Len(t, resources, 1)
	require.Equal(t, "stateSchemas", resources[0].Kind)
	require.Equal(t, "task", resources[0].ResourceKey)
	_, err = s.Approve("t", p.ID, "admin-1", map[string]any{"network": map[string]any{"outbound": []any{"*"}}})
	require.ErrorContains(t, err, "exactly match")
	approved, err := s.Approve("t", p.ID, "admin-1", nil)
	require.NoError(t, err)
