...
export default function GroupWorkspacePage() {
  ...
  const addMember = useGroupAction((values: { agentId: number; role: GroupMemberRole }) => groupApi.addMember(actualId!, values), '成员已加入群组')
  const addChannel = useGroupAction((values: { name: string; topic?: string; visibility: 'group' | 'members' }) => groupApi.addChannel(actualId!, values), '频道已创建')
  const addSession = useGroupAction((values: { agenda: string; hostAgentId?: number; participantAgentIds?: number[] }) => groupApi.createSession(activeChannelId, values), '会话房间已创建')
  const updateMember = useGroupAction((input: { agentId: number; role: GroupMemberRole }) => groupApi.updateMember(actualId!, input.agentId, input.role), '成员角色已更新')
