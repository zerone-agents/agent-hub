...
const useStyles = createStyles(({ css }) => ({
  ...
  danger: css`
    &:hover {
      color: ${t.danger};
      background: color-mix(in srgb, ${t.danger} 8%, transparent);
    }
  `,
}))
...
export default function RelationListPage() {
  ...
  const columns: ColumnsType<AgentRelation> = [
        ...
        <div className={styles.edge}>
          <div className={styles.agentNode}>
            <div className={styles.agentTitle}>
              {getAgentTitle(agents, relation.sourceAgentId, relation.sourceAgentName)}
            </div>
          ...
          <div className={styles.agentNode}>
            <div className={styles.agentTitle}>
              {getAgentTitle(agents, relation.targetAgentId, relation.targetAgentName)}
            </div>
    ...
    },
    {
      ...
      render: (_, relation) => (
        <div>
          {actionLabels(relation.allowedActions).map((label) => (
      ...
      render: (constraint: string) =>
        constraint ? (
          <Tooltip title={constraint}>
            <span className={styles.constraint}>{constraint}</span>
          </Tooltip>
        ) : (
