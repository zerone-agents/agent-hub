...
export const useWorkflow=(id?:string)=>useQuery({queryKey:['workflows',id],queryFn:async()=>{const data=unwrapResponse<{workflow:Workflow;versions:Workflow['versions']}>(await workflowApi.get(id!));return {...data.workflow,versions:data.versions}},enabled:Boolean(id)})
...
export const useWorkflowExecution=(id?:string)=>useQuery({queryKey:['workflow-executions',id],queryFn:async()=>unwrapResponse<WorkflowExecution>(await workflowApi.execution(id!)),enabled:Boolean(id),refetchInterval:3000})
export const useWorkflowAudit=(id?:string)=>useQuery({queryKey:['workflow-executions',id,'audit'],queryFn:async()=>list(unwrapResponse<WorkflowAudit[]>(await workflowApi.executionAudit(id!))),enabled:Boolean(id)})
...
export const useDecision=(id?:string)=>useQuery({queryKey:['decisions','detail',id],queryFn:async()=>unwrapResponse<CollectiveDecision>(await decisionApi.get(id!)),enabled:Boolean(id),refetchInterval:3000})
export const useDecisionAudit=(id?:string)=>useQuery({queryKey:['decisions','audit',id],queryFn:async()=>list(unwrapResponse<DecisionAudit[]>(await decisionApi.audit(id!))),enabled:Boolean(id)})
