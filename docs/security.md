...
4. **`tenant_oauth_clients.client_secret_enc` / `cert_enc` are AES-GCM encrypted at rest** (`PROVIDER_ENCRYPTION_KEY`); ops API responses never return secrets.
5. **CLI tokens carry the issuing user's full permissions** (`/api/v1/cli/*`, admin/maintainer only) — a leaked CLI token is equivalent to the user's session until expiry/revocation. Tokens are stored as SHA-256 hashes; revoke immediately on suspicion.
6. **CORS defaults to AllowAll + AllowCredentials** — production must explicitly configure allowlist.
7. **ServiceRouter reverse proxy has SSRF risk** — `EndpointURL` lacks scheme / private subnet validation.
8. **OAuth state stored in memory** — process restart loses in-flight logins; recommend migrating to Redis.
9. **Token Exchange logging** — currently prints response bodies containing tokens; should be removed in production.
