...
- [x] H7.1 扩展生命周期 — commit 90a1231（安装/启停/升级/回滚/卸载/依赖/迁移引擎/影响范围，17 新测试）
- [x] H7.2 UI 插槽 — 随 6ea204b（4 种声明式组件、代理白名单+限流、错误边界、5 宿主页挂载）
- [x] H7.3 模板库 — 随 6ea204b（spec 校验/预览/映射/冲突 rename/幂等/导出/seed 模板/安装向导）
- [x] H7.4 权限与隔离 — 随 6ea204b（grants auto/approval、Enforce 中间件、审计、限流、敏感字段掩码、11 项隔离矩阵）
- [x] H7.5 用量与运维 — 随 6ea204b（埋点零阻断、统计/预算/告警/健康/CSV 导出、SVG 趋势图）
- [x] H7.6 SDK/CLI — 随 6ea204b（zerone CLI 五命令、ed25519 签名验签、Go SDK、2 示例扩展、开发者指南）
- [x] 统一接线 + 全量测试 — go test ./... 全绿（31 包）、前端 518/518 + tsc + build 过
- [ ] H7.7 独立审查 → 部署 → 页面验收
- [ ] H7.2 UI 插槽
- [ ] H7.3 模板库
- [ ] H7.4 权限与隔离
