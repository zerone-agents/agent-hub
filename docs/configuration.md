...
**Agent graph 部署协议（issue #111）**：hub 部署 Agent 时向 agent-deployer 下发完整 Agent 图——`rootAgentId + agents[]`，subagent 为纯 id 引用，每个 Agent 携带自身完整能力（不继承、不回退父级配置），`maxSessionQueries` 等运行时全局字段仅 root 携带。链路版本要求：**agent-deployer ≥ v3.0.0（v3 破坏性契约，旧版对 graph 请求返回 400）+ agent-runtime ≥ v2.6.1 + agent-sdk ≥ v3.1.0**；hub 升级后需**重新部署存量 Agent**。
